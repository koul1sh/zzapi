...
* `body`: the raw request body. (Use the value `file://<filename>` to read from a file), or an object, which will be converted to a JSON string after variable replacements. If the value of any field is a file (specified using `file://<filename>`), the body will be sent as multipart/form-data, treating each key as a form field.
